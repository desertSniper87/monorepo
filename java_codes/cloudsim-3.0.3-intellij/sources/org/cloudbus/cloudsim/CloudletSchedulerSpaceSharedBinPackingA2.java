 ...
 */
public class CloudletSchedulerSpaceSharedBinPackingA2 extends CloudletScheduler {
    public final int bins = 10;
     ...
     * @post $none
     */
        public CloudletSchedulerSpaceSharedBinPackingA2() {
            super();
        ...
        public void binPack(){
            ...
            ArrayList<ArrayList<ResCloudlet>> binArray = new ArrayList<ArrayList<ResCloudlet>>(bins);
            int numberOfCloudlets = cloudletWaitingList.size();
            int cloudletToBeAdded = numberOfCloudlets;
//            for (int i=0;i<bins;i++){                // Consider 10 sets of bins
//                binArray.add(); = new ArrayList<ResCloudlet>();
//            }

            int perBinCloudlets = numberOfCloudlets/bins;
            int bin = 0;
            while (cloudletToBeAdded!=0 && bin<bins){
                for (int i = 0; i < perBinCloudlets; i++) {
                    Iterator<ResCloudlet> iterator = (Iterator<ResCloudlet>) cloudletWaitingList.iterator();

                    while(iterator.hasNext()){
                        ResCloudlet resCloudlet = iterator.next();

                        binArray.get(bin).add(resCloudlet);
                        cloudletToBeAdded--;
                    }
                }
                bin++;
            ...
            for (int i = 0; i<bins ; i++) {           // if the bins free space is in between i*0.1 and (i+1)*0.1
                    int random = new Random().nextInt(binArray.get(i).size());
                    ResCloudlet randomCloudlet = binArray.get(i).get(random);

                    for (int j = 0; j < bins; j++) {
                        random = new Random().nextInt(bins);
                        List randomBin = binArray.get(random);

                        if (randomBin.size() < perBinCloudlets){
                            randomBin.add(randomCloudlet);
                            binArray.get(i).remove(randomCloudlet);
                            break;
                        }
                    }
            }
            ...
            for (int i=0;i<bins;i++){
                Iterator<ResCloudlet> iterator = binArray.get(i).iterator();

                while(iterator.hasNext()){
                    ResCloudlet resCloudlet = iterator.next();

                    cloudletWaitingList.add(resCloudlet);
                }
            }
        }
    ...
    private <T extends ResCloudlet> List<T> getCloudletWaitingList() {
...
//    }
//
     protected <T extends ResCloudlet> void cloudletWaitingList(List<ResCloudlet> cloudletWaitingList) {
         this.cloudletWaitingList = cloudletWaitingList;
     }
