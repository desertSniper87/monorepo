    ...
    import org.cloudbus.cloudsim.util.WaitingListComparator;
     ...
     */
    public class CloudletSchedulerSpaceSharedBinPackingA1 extends CloudletScheduler {
        ...
        /**Cloudlet lists for implementing algorithm
         * We just dump generics for simple lists*/

        private List<ResCloudlet> cloudletWaitingListS;
        private List<ResCloudlet> cloudletWaitingListM1;
        private List<ResCloudlet> cloudletWaitingListM2;
        private List<ResCloudlet> cloudletWaitingListL;
        /** The current CPUs. */
         ...
         * @post $none
         */
        public CloudletSchedulerSpaceSharedBinPackingA1() {
            super();
            ...
            cloudletWaitingListS = new ArrayList<ResCloudlet>();
            cloudletWaitingListM1 = new ArrayList<ResCloudlet>();
            cloudletWaitingListM2 = new ArrayList<ResCloudlet>();
            cloudletWaitingListL = new ArrayList<ResCloudlet>();
        ...
        public void binPack(List<ResCloudlet> cloudletWaitingList){
            int i;
            int bin_counter = 0;
            System.out.println(cloudletWaitingList.size());

            for (i = 0; i < .33*cloudletWaitingList.size(); i++) {
                cloudletWaitingListS.add(cloudletWaitingList.get(i));
            }
            for (; i < .5*cloudletWaitingList.size(); i++) {
                cloudletWaitingListM1.add(cloudletWaitingList.get(i));
            }
            for (;i < .67*cloudletWaitingList.size(); i++) {
                cloudletWaitingListM2.add(cloudletWaitingList.get(i));
            }
            for (;i < cloudletWaitingList.size(); i++) {
                cloudletWaitingListL.add(cloudletWaitingList.get(i));
            }

            cloudletWaitingList.clear();

//            Collections.sort(cloudletWaitingListM1, new WaitingListComparator());
//            Collections.sort(cloudletWaitingListM2, new WaitingListComparator());

            List<Long> M1ExecutionTime = new ArrayList<Long>();

            for (ResCloudlet c1: cloudletWaitingListM1){
                M1ExecutionTime.add(c1.getCloudletLength());
            }
            Collections.reverse(M1ExecutionTime);

            for (ResCloudlet c2: cloudletWaitingListM2){                    // for any item a in M2
                if (M1ExecutionTime.contains(c2.getCloudletLength())){      // if a can be matched with at least an item in M1 // Match a with the biggest possible item in M1
                    //M1ExecutionTime.remove(c2.getCloudletLength());         // Eliminate them
                    //cloudletWaitingListM2.remove(c2);
                    cloudletWaitingList.add(c2);
                    bin_counter++;
                }
            }
            bin_counter += (cloudletWaitingListM1.size())/2;

//            List<Long> M2ExecutionTime = new ArrayList<Long>();
//            for (ResCloudlet c2: cloudletWaitingListM2){
//                M2ExecutionTime.add(c2.getCloudletLength());
//            }

            for (ResCloudlet a: cloudletWaitingListM2){                      // any item a in M2
                for (ResCloudlet b: cloudletWaitingListS) {                  // choose an item b in S
                    ResCloudlet c = b;                                       // c = b
                    //cloudletWaitingListS.remove(b);                          // eliminate b
                    cloudletWaitingList.add(b);
                }
                //cloudletWaitingListM2.remove(a);                             // eliminate a
                cloudletWaitingListS.add(a);                                 // put c in s
                bin_counter++;
            }

//            while (!cloudletWaitingListS.isEmpty()){
//                for (ResCloudlet a: cloudletWaitingListS){
//                    for (ResCloudlet b : cloudletWaitingListS) {
//                        cloudletWaitingList.add(a);
//                        cloudletWaitingListS.remove(a);
//                        cloudletWaitingList.add(b);
//                        cloudletWaitingListS.remove(b);
//                    }
//                }

//                Iterator<ResCloudlet> iterator = cloudletWaitingListS.iterator();


//            }
            Iterator<ResCloudlet> iterator = cloudletWaitingListS.iterator();
            while (iterator.hasNext()){
                ResCloudlet a = iterator.next();
//                cloudletWaitingListS.remove(a);
                cloudletWaitingList.add(a);
            }


            for (ResCloudlet l : cloudletWaitingListL){
                cloudletWaitingList.add(l);
            }
        ...
        public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
            ...
            //getCloudletExecList().POINT

            binPack((List<ResCloudlet>) cloudletWaitingList);

            for (ResCloudlet i : getCloudletExecList())
                Log.printLine(i.getCloudletLength());

            //Log.printLine(getCloudletExecList().);
            ...
            if (!getCloudletWaitingList().isEmpty()) {                          // Waiting list is not empty
                for (int i = 0; i < finished; i++) {
                    toRemove.clear();
                    //binPack(cloudletWaitingList);
            ...
            for (ResCloudlet rcl : getCloudletExecList()) {
                ...
                if (estimatedFinishTime < nextEvent) {
                    nextEvent = estimatedFinishTime;            // Shortest job first
                }
            }
            setPreviousTime(currentTime);
            return nextEvent;
        ...
        /**
        *   Now we create A, M1, M2, L
        * */

        protected <T extends ResCloudlet> List<T> getCloudletWaitingListS() {
                    return (List<T>) cloudletWaitingListS;
                }
        protected <T extends ResCloudlet> List<T> getCloudletWaitingListM1() {
                    return (List<T>) cloudletWaitingListM1;
                }
        protected <T extends ResCloudlet> List<T> getCloudletWaitingListM2() {
                    return (List<T>) cloudletWaitingListM2;
                }
        protected <T extends ResCloudlet> List<T> getCloudletWaitingListL() {
                    return (List<T>) cloudletWaitingListL;
                }
