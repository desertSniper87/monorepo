...
public class TranscodingMain {
	...
	private static TranscodingDatacenter datacenter0;
	//private static TranscodingDatacenter datacenter1;
...
//////////////////////////STATIC METHODS ///////////////////////
	...
	*/
    ...
    //String[] args = {"-property", "/Users/torsho/Dropbox/Programming Code/Github projects/Simulation/resources/properties/config.properties",
    String[] args = {"-property", "properties/config.properties",
            //"-input", "/home/torsho/Dropbox/Programming Code/Github projects/Simulation/resources/inputdata/",
            "-input", "inputdata/",
            //"-output", "/home/torsho/Dropbox/Programming Code/Github projects/Simulation/resources/outputdata/test.txt",
            "-output", "outputdata/test.txt",
            "-sortalgorithm", "",
            "-startupqueue",
            "-stqprediction",
            "-videonum", "100",
	...
	try {
		...
		output = new FileOutputStream("modules/cloudTranscodingApp/config.properties");
		//output = new FileOutputStream(propertiesFileURL);
		/**
		 * Configuration properties for datacenter0
		 */
        ...
        File folder = new File("inputdatafile");
      ...
      //  int jobNum = listOfFiles.length;
		...
		/**
		 * configuration properties in broker and datacenter0's VM local queue size
		 */
	...
	try {
		...
		//choose datacenter0 as the datacenter0 to create new vm, which can improve later
		TranscodingMain.datacenter0 = datacenter0;
		//unused
		//TranscodingMain.datacenter1 = datacenter1;
	 ...
	 //   public boolean generatePeriodicEvent = true; //true if new internal events have to be generated
		...
		public void processNewJobs(){
		        ...
		        try {
		            ...
		            for (int i = 0; i < 1; i++) {
		            	int cloudlets = (int)getRandomNumber(10, 60, random);
...
//		                System.out.println(result.get() + " End.");
		...
		public void processProvisioning(){
			...
			if(vmNum == 0 && !prediction){
				...
				for(Vm vm_rt : datacenter0.getVmList()){
				...
				try {
					...
					System.out.println("The VMList size is: " + datacenter0.getVmList().size());
					...
					if(allcoationDeallocationFlag == 1 ){
						if(datacenter0.getVmList().size() < MAX_VM_NUM) {
							//Create a new vm
							...
							//creat a event for datacenter0 to create a vm
							sendNow(datacenter0.getId(),CloudSimTags.VM_CREATE_ACK, vmNew.get(0));
						}else{
						...
						if(datacenter0.getVmList().size() < MAX_VM_NUM) {
							...
							//creat a event for datacenter0 to create a vm
							sendNow(datacenter0.getId(),CloudSimTags.VM_CREATE_ACK, vmNew1.get(0));
							...
							//creat a event for datacenter0 to create a vm
							sendNow(datacenter0.getId(),CloudSimTags.VM_CREATE_ACK, vmNew2.get(0));
						...
						int vmIndex = 0;
						int runningVmNum = 0;
						for(Vm vm : datacenter0.getVmList()){
						...
						if(datacenter0.getVmList().size() > 1 && runningVmNum >1){
