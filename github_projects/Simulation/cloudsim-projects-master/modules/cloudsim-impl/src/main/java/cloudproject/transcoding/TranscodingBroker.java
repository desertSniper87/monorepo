...
public class TranscodingBroker extends DatacenterBroker{
	...
	protected List<VideoSegment> lstCloudlet = new ArrayList<VideoSegment>();
   ...
   //video Id
	...
	protected void submitCloudlets() {
		...
		else if (sortalgorithm.equals("BPA1")){
		    sortedByBinPackA1();
        }

		else if (sortalgorithm.equals("BPA2")){
		    sortedByBinPackA2();
        }
        ...
        * 2. find the smallest completion time vm, and send cloudlet to it.
        */
					      ...
					      //  vstream.getNewArrivalQueue().remove(0);
						    ...
						    if(!displayStartupTimeMap.containsKey(cloudlet.getCloudletVideoId()) || cloudlet.getCloudletId() == 0){	
								...
								// cloudlet.setCloudletDeadline(minToCompleteTime);
								 ...
								 for(VideoSegment vs: lstCloudlet){
									 if(vs.getCloudletVideoId() == cloudlet.getCloudletVideoId()){
									    vs.setCloudletDeadline(vs.getCloudletDeadlineAfterPlay() + displayStartupTimeMap.get(vs.getCloudletVideoId()));	
									 }
								 }	 
		...
		protected void sortedBySJF() {
	      ...
	      //  System.out.println("\n\t PRIORITY  Broker Schedules\n");
	       // System.out.println("Before ordering");
            ...
            for (int i = 0; i < lstCloudlet.size(); i++) {
                System.out.println(lstCloudlet.get(i).getCloudletId()+" "+executionTimeList.get(i));
            }
            setCloudletList(lstCloudlet);
...
//            for (VideoSegment cloudLet: lstCloudlet){
//                cloudLet.setExecutionTime(vmList);
//            }
//            lstCloudlets.sort(new ExecutionTimeComparator());
//            setCloudletList(lstCloudlets);
			
		}

    /** My scheduling code A1*/

    public void sortedByBinPackA1(){
        lstCloudlet = getCloudletList();
        if (lstCloudlet.size()>=4) {
            int i = 0;
            int bin_counter = 0;
            double totalExecutionTime = 0;
            for (VideoSegment cloudLet: lstCloudlet){
                cloudLet.setExecutionTime(vmList);
                totalExecutionTime += cloudLet.getExecutionTime();
            }
            //Collections.sort(lstCloudlet, new ExecutionTimeComparator());
            sortedBySJF();
            Collections.reverse(lstCloudlet);

            List <VideoSegment> cloudletListS = new ArrayList<VideoSegment>();
            List <VideoSegment> cloudletListM1 = new ArrayList<VideoSegment>();
            List <VideoSegment> cloudletListM2 = new ArrayList<VideoSegment>();
            List <VideoSegment> cloudletListL = new ArrayList<VideoSegment>();

            for (i = 0; i < .33* lstCloudlet.size(); i++) {
                cloudletListS.add(lstCloudlet.get(i));
            }
            for (; i < .5* lstCloudlet.size(); i++) {
                cloudletListM1.add(lstCloudlet.get(i));
            }
            for (; i < .67* lstCloudlet.size(); i++) {
                cloudletListM2.add(lstCloudlet.get(i));
            }
            for (; i < lstCloudlet.size(); i++) {
                cloudletListL.add(lstCloudlet.get(i));
            }
...
//            i = 0;
//            VideoSegment[] cloudletArrayM1 = new VideoSegment[cloudletListM1.size()];
//            for (VideoSegment c : cloudletListM1){
//                cloudletArrayM1[i] = c;
//                i++;
//            }
...
//                VideoSegment cloudletS = iteratorS.next();
//                lstCloudlet.add(cloudletS);
////                lstCloudlet.add(cloudletArrayM1[i]);
//                iteratorS.remove();
//                i++;
//            }

//            for (;i<cloudletArrayM1.length;i++)
//                lstCloudlet.add(cloudletArrayM1[i]);

//            double executionTimeAverage = totalExecutionTime/lstCloudlet.size();
//            VideoSegment cloudlet = iteratorS.next();



            lstCloudlet.addAll(cloudletListS);
            lstCloudlet.addAll(cloudletListM1);
//
...
//            Collections.sort(cloudletListL, new ExecutionTimeComparator());
            ...
            lstCloudlet.addAll(cloudletListM2);
            lstCloudlet.addAll(cloudletListL);
    ...
    /** My scheduling code A2*/

    protected void sortedByBinPackA2() {
        sortedBySJF();
//        sortedByFCFS();
//        sortedbyDeadline();
        List <VideoSegment>lstcloudlet = getCloudletList();
        if (lstcloudlet.size()>1) {
//            bins = lstcloudlet.size() / 2;

            for (VideoSegment cloudLet: lstcloudlet){
                cloudLet.setExecutionTime(vmList);
            }

            Iterator<? extends Cloudlet> iteratorMainList = lstcloudlet.iterator();
            long totalLength = 0;
            while (iteratorMainList.hasNext()) {
                VideoSegment cloudlet = (VideoSegment) iteratorMainList.next();
                totalLength += cloudlet.getExecutionTime();
            }

            double[] weight = new double[lstcloudlet.size()];
            int j = 0;
            for (VideoSegment cloudlet: lstcloudlet){
                weight[j] = cloudlet.getExecutionTime();
                j++;
            }
            int resultBins = 0;
            int maxBins = lstcloudlet.size()/2;
            double capacity = totalLength/maxBins;
            double[] binRemainingSpace = new double[maxBins];

            for (int i=0;i<maxBins;i++){
                for (j=0;j<resultBins;j++){
                    if (binRemainingSpace[j]>=weight[i]){
                        binRemainingSpace[j] = binRemainingSpace[j]-weight[i];
                         break;
                    }
                }

                if (j == resultBins){
                    binRemainingSpace[resultBins] = capacity - weight[i];
                    resultBins++;
                }
            }
            ...
            if (bins>3){
                //ArrayList<ResCloudlet> binArray = new ArrayList<ResCloudlet>[bins]();
                ArrayList<ArrayList<VideoSegment>> tempBinArray = new ArrayList<ArrayList<VideoSegment>>(bins);
                ArrayList<ArrayList<VideoSegment>> binArray = new ArrayList<ArrayList<VideoSegment>>(bins);
                int numberOfCloudlets = lstcloudlet.size();
                //int cloudletToBeAdded = numberOfCloudlets;
                for (int i = 0; i < bins; i++) {                // Consider 10 sets of bins
                    tempBinArray.add(new ArrayList<VideoSegment>());
                    binArray.add(new ArrayList<VideoSegment>());
                }

                int bin = 0;
                long avgBinSpace = totalLength / bins;
                long tempBinSpace = 0;
                iteratorMainList = lstcloudlet.iterator();

                while (iteratorMainList.hasNext()) {
                    VideoSegment cloudlet = (VideoSegment) iteratorMainList.next();
                    tempBinArray.get(bin).add(cloudlet);
                    tempBinSpace += cloudlet.getExecutionTime();
    //                if (tempBinSpace>avgBinSpace*bin){
                    if (tempBinSpace > avgBinSpace) {
    //                    tempBinArray.get(bin).sort(new ExecutionTimeComparator());
                        bin++;
                        tempBinSpace = 0;
                    }
                ...
                ArrayList<Double> spaceArray = new ArrayList<Double>(bins);
                for (int i = 0; i < bins; i++) {
                    spaceArray.add(i, 0D);
                }

                for (int tempArrayBinNo = 0; tempArrayBinNo < bins; tempArrayBinNo++) {           // if the bins free space is in between i*0.1 and (i+1)*0.1
                    while (!tempBinArray.get(tempArrayBinNo).isEmpty()) {
                        int randomTempBinArrayCloudLetNum = new Random().nextInt(tempBinArray.get(tempArrayBinNo).size());
                        int randomBin = new Random().nextInt(binArray.size());
                        VideoSegment randomCloudlet = tempBinArray.get(tempArrayBinNo).get(randomTempBinArrayCloudLetNum);
                        if (spaceArray.get(randomBin) + randomCloudlet.getExecutionTime() < avgBinSpace * randomBin) {
                            binArray.get(randomBin).add(randomCloudlet);
                            tempBinArray.get(tempArrayBinNo).remove(randomCloudlet);
                            double newCloudletLength = spaceArray.get(randomBin) + randomCloudlet.getExecutionTime();
                            spaceArray.set(randomBin, newCloudletLength);
    //                        tempBinArray.get(randomBin).sort(new ExecutionTimeComparator());
                        }
                    }
                }
                ...
                for (ArrayList<VideoSegment> list : binArray){
                    lstcloudlet.addAll(list);
                }
        ...
        //Collections.reverse(lstCloudlet);
        for (VideoSegment cloudlet : lstcloudlet)
            System.out.println(cloudlet.getCloudletId()+" "+cloudlet.getExecutionTime());
          setCloudletList(lstcloudlet);
    }

/*    protected void sortedByBinPackA2() {
        lstCloudlet = getCloudletList();
        if (!lstCloudlet.isEmpty()) {
            bins = lstCloudlet.size() / 2;

            *//*for (VideoSegment cloudLet: lstCloudlet){
                cloudLet.setExecutionTime(vmList);
            }*//*
            Collections.sort(lstCloudlet, new WaitingListComparator());
            //Collections.reverse(lstCloudlet);
            Iterator<? extends Cloudlet> iteratorMainList = lstCloudlet.iterator();
            long totalLength = 0;
            while (iteratorMainList.hasNext()) {
            ...
            while (iteratorMainList.hasNext()) {
                VideoSegment cloudlet = (VideoSegment) iteratorMainList.next();
                tempBinArray.get(bin).add(cloudlet);
                tempBinSpace += cloudlet.getCloudletLength();
//                if (tempBinSpace>avgBinSpace*bin){
                if (tempBinSpace > avgBinSpace) {
                    bin++;
                    tempBinSpace = 0;
            ...
            for (int tempArrayBinNo = 0; tempArrayBinNo < bins; tempArrayBinNo++) {           // if the bins free space is in between i*0.1 and (i+1)*0.1
                while (!tempBinArray.get(tempArrayBinNo).isEmpty()) {
                    ...
                    int randomBin = new Random().nextInt(binArray.size());
                    VideoSegment randomCloudlet = tempBinArray.get(tempArrayBinNo).get(randomTempBinArrayCloudLetNum);
                    if (spaceArray.get(randomBin) + randomCloudlet.getCloudletLength() < avgBinSpace * randomBin) {
                        binArray.get(randomBin).add(randomCloudlet);
                        tempBinArray.get(tempArrayBinNo).remove(randomCloudlet);
                        double newCloudletLength = spaceArray.get(randomBin) + randomCloudlet.getCloudletLength();
                        spaceArray.set(randomBin, newCloudletLength);
                    }
                }
            }
