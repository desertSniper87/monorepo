(* ----------------------------------------------------------------------------
 ...
 hori@ingjapan.or.jp   or    TRSOFT@Menden.net

  Original

    Copyright © 1998 by H.Hori  All Rights Reserved.
    E-Mail: hori@ingjapan.or.jp
    Homepage: http://www.ingjapan.or.jp/hori

  Translation

    Copyright © 1998 by TRSOFT  All Rights Reserved.
    Thomas Radtke Software Entwicklung.
    E-Mail: TRSOFT@Menden.net
    http://www.abcnet.de/TRSOFT/

  THIS SOFTWARE IS PROVIDED AS IS AND WITHOUT WARRANTY OF ANY KIND,
  EITHER EXPRESSED OR IMPLIED.  Version 1.05a from 10.03.98-----------------------------------------------------------------------------*)
...
begin
  if not FOpened then
    raise EWaveStreamError.Create('Œ»ÝŠJ‚©‚ê‚Ä‚¢‚Ü‚¹‚ñ');
...
begin
  if not FOpened then
    raise EWaveStreamError.Create('Can`t write Wave');
...
begin
  if FOpened then
    raise EWaveStreamError.Create('Allways Open');
  ...
  if (WF.FType<>ID_RIFF) and (WF.RType<>ID_WAVE) then
    raise EWaveStreamError.Create('Invalid Wave ');
...
begin
  ...
  if FormatSize=0 then
    raise EWaveStreamError.Create('Invalid Wave Format ');
