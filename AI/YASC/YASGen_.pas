...
-----
  ...
  * YASGen -ga -width 6 -height 6 -walls 10 -boxes 6 -maxboxes 6 -maxopen 100000 -tail 1 -seed 123456
  ...
  * YASGen -ga -file template.sok -boxes 4 -maxboxes 8 -seed 123456

      Adds 4 boxes to the level in 'template.sok' and uses it as seed for
      the genetic generator.

  * YASGen -ga -file template.sok -fixed -type random -memory 128
  ...
  * YASGen -ga -forward -file template.sok -fixed
...
{ Before compiling the program, there are 4 choices to make:
  ...
  * Application Type...: Console, Plugin, or Silent (no console, and no graphical user interface either)
  * Output.............: Verbose of brief
  ...
  Compiling the source-file as a plugin produces a generator module ready to be
  embedded in Sokoban YASC.
...
{///$DEFINE CONSOLE_APPLICATION}               {application type: select CONSOLE, PLUGIN, or none}
{or}
{$DEFINE YASC}                                 {generator module for Sokoban YASC}
{or}
...
{$IFNDEF YASC}
  Program YASGen;                              {YASGen - Yet Another Sokoban Level Generator - For Small Levels}
...
interface
  uses Windows,SysUtils,
       Text_,SokUtil_,SokFile_,SokGame_;
...
type
  TDirection               = (dUp,dLeft,dDown,dRight); {caution: must match 'DIRECTION_BIT_COUNT' defined below}
...
const
  ...
  {$ENDIF}                                     {additional board-square flags}
  ...
  DEFAULT_RANDOM_BOARD_HEIGHT
                           = 9;
  DEFAULT_RANDOM_BOARD_INTERIOR_WALLS_PERCENT
                           = 40;
  DEFAULT_RANDOM_BOARD_WIDTH
                           = DEFAULT_RANDOM_BOARD_HEIGHT;
...
type
  ...
  TBoard2D                 = {$IFDEF YASC}
                               SokFile_.TBoard;
                             {$ELSE}
                               array[0..MAX_BOARD_WIDTH+1,0..MAX_BOARD_HEIGHT+1] of Byte;
                             {$ENDIF}
  TBoardBoolean            = array[0..MAX_BOARD_SIZE] of Boolean;
  TBoardInteger            = array[0..MAX_BOARD_SIZE] of Integer;
  TBoardTextLines          = array[0..MAX_BOARD_HEIGHT] of String[MAX_BOARD_WIDTH];
  THashValue               = Int64;
  THistory                 = record
    ...
    {$IFDEF YASC}
      SecondaryMetrics     : TSecondaryScoreMetrics;
    {$ENDIF}
...
const
  GA_DEFAULT_AUTO_SAVE_INTERVAL_TIME_MS
                           = 10 {minutes} * 60 {seconds} * ONE_THOUSAND {milliseconds}; {$IFDEF YASC} {must match the text '(Every <number> minutes' on the settings window} {$ENDIF}
  ...
  GA_MAX_PENDING_CANDIDATES= 50; {in Sokoban YASC, the user can inject new candidates via the editor while the generator is running}

{GA Texts} {don't localize}

  GA_TEXT_APPLICATION_TITLE_LONG
                           = TEXT_APPLICATION_TITLE+' - Yet Another Sokoban Level Generator';
  ...
  GA_TEXT_FILE_HEADER_LINES= 'Do not edit this file manually. It contains a Sokoban level candidate set '+
                             'for the YASGen level generator. The file must be formatted '+
...
type
  ...
  TGAIndividual            = record
    ...
    {$IFDEF YASC}
      Level                : TLevel;     {the attached level in 'MainForm.Generator.SokoFile'}
    {$ENDIF}
  ...
  TGAControl               = record   {setup info}
    ...
    {$IFDEF YASC}
      LastShowStatusTimeMS : TTimeMS; {last time the screen was updated with progress information}
    {$ENDIF}
//  MaxCandidateCount      : Integer; {stop criteria}
  ...
  TGA                      = record
    ...
    {$IFDEF YASC}
      ParentLevel1         : TLevel;  {parents of the currently processed candidate}
      ParentLevel2         : TLevel;
    {$ENDIF}
...
{$IFDEF YASC}
  ...
  function  GAAppendFromSokoFile(SokoFile__:TSokoFile):Boolean;
  function  GAAppendFromGame(Game__:TSokoGame; IsACandidateSetMember__,UseCandidateNumbers__:Boolean):Boolean;
  function  GACalculateElapsedTimeMS:TTimeMS;
  procedure GAClear;
  procedure GADeleteIndividual(IndividualNo__:Integer);
  function  GAFitnessValue(FitnessFunction__:TGAFitnessFunction; PushCount__,Score__:Integer):Integer;
  procedure GAFitnessValueToPushesAndScore(FitnessFunction__:TGAFitnessFunction; Value__:Integer; var PushCount__,Score__:Integer); {the fitness push count is the solution path length minus discounted trivial opening and closing pushes}
  function  GAIndividualWithHighestChildCountExceptTheBest:Integer;
  function  GALookupLevel(Level__:TLevel; var Index__:Integer):Boolean;
  procedure GARun;
  function  GASaveToFile(const FileName__:String):Boolean;
  ...
  function  Random(Range__:Integer):Integer;

  implementation

  uses Misc_,Open1_,Tools_,Main_,Snapshots_,Plugin_,Generator_;
...
function  BoardToText(BoardWidth__,BoardHeight__:Integer; const Board__:TBoard; const LineSeparator__:String):String; forward;
...
function  ColRowToSquare(Col__,Row__,ColCount__,RowCount__:Integer):Integer; forward;
...
function  LoadNextLevelFromFile(MinimumBoardWidth__,MinimumBoardHeight__:Integer):Boolean; forward;
...
procedure MakeBoard(BoardWidth__,BoardHeight__:Integer); forward;
function  MakeLevelFrom2DimensionalBoard(BoardWidth__,BoardHeight__,MinBoardWidth__,MinBoardHeight__:Integer; const Board2D__:TBoard2D):Boolean; forward;
...
//function  GAIdenticalChromosomes(const Alleles1__,Alleles2__:TGAAlleles):Boolean; forward;
{$IFDEF YASC}
  function  GAMakeAllelesFromLevel(IndividualNo__:Integer):Boolean; forward;
{$ELSE}
...
var Col,Row,RowOffset,Index:Integer;
begin
  ...
  if   Result then begin
       ...
       for Row:=1 to Game.BoardHeight do begin
           RowOffset:=ColRowToSquare(0,Row,Game.BoardWidth,Game.BoardHeight);
...
var Col,Row,RowOffset,Index:Integer; Character:Char;
begin
  Index:=0;
...
begin
  if   (IndividualNo__>=1) and (IndividualNo__<=GA.IndividualCount) then with GA.Individuals[IndividualNo__] do begin
       {$IFDEF YASC}
         if        Assigned(Level) and (Level.Name<>'') then
                   Result:=Level.Name
         else if   CandidateNo<>0 then
                   Result:=CandidateText+SPACE+IntToStr(CandidateNo)
              else Result:=CandidateText+SPACE+IntToStr(IndividualNo__);
       {$ELSE}
...
procedure GADeleteIndividual(IndividualNo__:Integer);
var i:Integer;
begin {Deletes an individual from the population; YASC: an attached level in 'MainForm.Generator.SokoFile' is not deleted}
  with GA do with Control do begin
    ...
    {$IFDEF YASC}
      Individuals[IndividualCount].Level:=nil;
    {$ENDIF}
    Dec(IndividualCount);
    if IndividualCount=0 then BestIndividualNo:=0;
    WorstIndividualNo:=Min(IndividualCount,PopulationSize);
    {$IFDEF YASC}
      while (WorstIndividualNo>0) and (Individuals[WorstIndividualNo].Fitness<0) do
        Dec(WorstIndividualNo); {the worst number denotes the last calculated candidate on the list; the currently processed candidate and the user's manually added candidates are located after the calculated members of the population}
    {$ENDIF}
    end;
end;
...
{$IFDEF YASC}
  ...
  begin
    ...
    if Assigned(Level__) then begin
       Index__:=Succ(Level__.Flags-ToolsForm.GeneratorTaskQueue.StringGrid.FixedRows);
       if (Index__>=1) and (Index__<=GA.IndividualCount) and (GA.Individuals[Index__].Level=Level__) then begin {'True': found}
          end
       else begin {try harder; search through all the individuals for a match}
          Index__:=1;
          while (Index__<=GA.IndividualCount) and (GA.Individuals[Index__].Level<>Level__) do
            Inc(Index__);
          end;
       end;
    Result:=(Index__>=1) and (Index__<=GA.IndividualCount);
  end;
{$ENDIF}
...
procedure GAAddBoardToCandidateSet(IndividualNo__:Integer; var OldBoardWidth__,OldBoardHeight__:Integer);
var Index:Integer;

  procedure ResizeAlleles(OldBoardWidth__,OldBoardHeight__,NewBoardWidth__,NewBoardHeight__:Integer; NormalizeBoard__:Boolean; var Alleles__:TGAAlleles);
  var Col,Row,Index,AllelesLength:Integer; Board2D:TBoard2D;
  begin {resizes alleles (a board) and installs the new dimensions as global values for all candidates}
    GA.AllelesCount:=NewBoardWidth__*NewBoardHeight__; {update the number of alleles in the chromosomes}
    AllelesLength:=Length(Alleles__);

    if (AllelesLength<>0) and
       ((OldBoardWidth__<>NewBoardWidth__) or (OldBoardHeight__<>NewBoardHeight__) or (AllelesLength<>GA.AllelesCount)) then begin
       for Row:=0 to MAX_BOARD_HEIGHT+1 do
           for Col:=0 to MAX_BOARD_WIDTH+1 do Board2D[Col,Row]:=WALL; {first fill the board with walls}
       Index:=0;
       for Row:=1 to OldBoardHeight__ do {then unpack the alleles from their 1-dimensional vector to a 2-dimensional board}
           for Col:=1 to OldBoardWidth__ do begin
               Inc(Index);
               if Index<=AllelesLength then Board2D[Col,Row]:=LevelReader.CharToBoardSquareValue[Alleles__[Index]];
               end;

       {pack the 2-dimensional board to the 1-dimensional vector, using the new board dimensions}
       ...
       for Row:=1 to NewBoardHeight__ do
           for Col:=1 to NewBoardWidth__ do begin
               Inc(Index); Alleles__[Index]:=BoardSquareValueToChar(Board2D[Col,Row]);
               end;
       end;

    if (Game.BoardHeight<>NewBoardHeight__) or (Game.BoardWidth<>NewBoardWidth__) then begin
       Game.BoardHeight:=NewBoardHeight__; Game.BoardWidth:=NewBoardWidth__; {update the global game board dimensions}
       InitializeBoard(Game.BoardWidth,Game.BoardHeight,True); {calculate internal data, e.g., 'Game.BoardSize'}
       end;
  ...
  procedure ResizeTemplate(OldBoardWidth__,OldBoardHeight__,NewBoardWidth__,NewBoardHeight__:Integer);
  var Square:Integer; Alleles:TGAAlleles;
  begin
    InitializeBoard(OldBoardWidth__,OldBoardHeight__,False); // restore the old board dimensions, matching the template dimensions
    GA.AllelesCount:=OldBoardWidth__*OldBoardHeight__;
    Alleles:=SPACE;
    GABoardToAlleles(Generator.Board,Alleles);
    ResizeAlleles(OldBoardWidth__,OldBoardHeight__,NewBoardWidth__,NewBoardHeight__,False,Alleles);
...
begin {'GAAddBoardToCandidateSet'; creates alleles matching the current game state and assigns them to the candidate number 'IndividualNo__'}
  with GA do with Individuals[IndividualNo__] do begin
    ...
    Alleles:=BoardToText(Game.BoardWidth,Game.BoardHeight,Game.Board,'');

    if (Game.BoardWidth<>OldBoardWidth__) or (Game.BoardHeight<>OldBoardHeight__) then begin // 'True': the new candidate board has different dimensions than the boards belonging to the existing candidates
       ResizeAlleles(Game.BoardWidth,Game.BoardHeight,Max(OldBoardWidth__,Game.BoardWidth),Max(OldBoardHeight__,Game.BoardHeight),True,GA.Individuals[IndividualNo__].Alleles); // resize the new candidate
       if (OldBoardWidth__<>Game.BoardWidth) or (OldBoardHeight__<>Game.BoardHeight) then begin // 'True': the new candidate has bigger board dimensions than the existing ones; resize all candidates to the new bigger dimensions
          for Index:=1 to Max(IndividualNo__,IndividualCount) do
              if Index<>IndividualNo__ then ResizeAlleles (OldBoardWidth__,OldBoardHeight__,Game.BoardWidth,Game.BoardHeight,True,Individuals[Index].Alleles); // resize old candidates, if any
          if  Generator.FixedTemplate  then ResizeTemplate(OldBoardWidth__,OldBoardHeight__,Game.BoardWidth,Game.BoardHeight); // resize template
          end;
       end;
    GA.AllelesCount:=Game.BoardWidth*Game.BoardHeight;
    ...
    OldBoardWidth__:=Game.BoardWidth; OldBoardHeight__:=Game.BoardHeight; // update the current board dimensions for the candidate set members
    end;
end;
  ...
  with GA do with Individuals[IndividualNo__] do begin
    if IsACandidateSetMember__ then begin
       ...
       if ExtractKeyValue(s,GA_TEXT_FITNESS_FUNCTION_TYPES[ffPushes],s) and StrToInt(s,i) and
          ...
          PushCount:=i;
          {$IFDEF YASC}
            if not Assigned(Level) then // if there is an attached level in 'MainForm.Generator.SokoFile' then the fitness has already been calculated
          {$ENDIF}
       ...
       if ExtractKeyValue(s,GA_TEXT_FITNESS,s) and StrToInt(s,i) and
          ...
          {$IFDEF YASC}
            if (not Assigned(Level)) or (Fitness<=0) then // if there is an attached level in 'MainForm.Generator.SokoFile' then the fitness has already been calculated
          {$ENDIF}
               Fitness:=i;
          end;
...
{$IFDEF YASC}
  function  GAAppendFromSokoFile(SokoFile__:TSokoFile):Boolean;  {Specific to Sokoban YASC}
  var IndividualNo:Integer; IsAClipboardImport,IsANewCandidateSet:Boolean; List:SokUtil_.TList; Level:SokFile_.TLevel;
{$ELSE}
...
var
  BoardWidth,BoardHeight,CandidateNo:Integer;
  ...
  var
    ...
    begin
      {$IFDEF YASC}
        Result:=StrEqual(Text__,TEXT_NO_YES[True]);
      {$ELSE}
                  ...
                  if s[1]=LEFT_BRACKET then begin
                     ...
                     case ParseState of
                       ...
                       psFixedTemplate:    begin
                                             ...
                                             if   LoadNextLevelFromFile(BoardWidth,BoardHeight) then begin
                       ...
                       psCandidate:        begin
                                             if   CandidateNo<High(Individuals) then begin
                                                  ...
                                                  Result:=LoadNextLevelFromFile(BoardWidth,BoardHeight);
                                                  Generator.MaxBoxCount:=oMaxBoxCount; {restore maximum number of boxes}

                                                  if Result then begin
                                                     GAAddBoardToCandidateSet(CandidateNo,BoardWidth,BoardHeight);
...
begin {$I-} {GAAppendFromSokoFile / GALoadFromFile}
  with GA do begin
    BoardWidth:=0; BoardHeight:=0; CandidateNo:=0;
    ParseState:=psNone;

    {$IFDEF YASC}
      Result:=True;
      FillChar(TemplateBoard,SizeOf(TemplateBoard),0);
      //Control.PopulationSize:=GA_MAX_INDIVIDUALS;
      //Generator.MaxBoxCount:=MAX_BOX_COUNT;  // so existing boards aren't truncated more than necessary
      IsANewCandidateSet:=IndividualCount=0;
      IsAClipboardImport:=IsANewFileName(SokoFile__.Name); // if the file name is new then the file is a clipboard import; in that case, don't import solutions and candidate numbers

      List:=SokoFile__.FileHeader.Lines; LevelReader.InputLines:=List.First; // first parse the file notes for settings and statistics
      LevelReader.CurrentTextLine:='';
      while Result and Assigned(List) do begin
        if LevelReader.CurrentTextLine='' then begin // 'True': no line in the level reader buffer
           if not Assigned(LevelReader.InputLines) then begin // 'True' end of the current list of text lines
              if        List=SokoFile__.FileHeader.Lines then begin
                        List:=SokoFile__.Levels;
                        if not List.IsEmpty then begin
                           LevelReader.InputLines:=TLevel(List.Last).Notes.Lines.First; // parse notes belonging to the last level, if any (tested candidates may be listed after the active candidate set)
                           ParseState:=psNone;
                           end;
                        end
              else if   (List=SokoFile__.Levels) and (not List.IsEmpty) then begin
                        List:=TLevel(List.Last).SnapshotsAsText;
                        if not List.IsEmpty then begin
                           LevelReader.InputLines:=TSnapshotAsText(List.Last).Notes.Lines.First; // parse notes belonging to the last solution of the last level, if any (some of the tested candidates may be listed after the active candidate set)
                           ParseState:=psNone;
                           end;
                        end
                   else List:=nil;
           ...
           if Assigned(LevelReader.InputLines) then begin // 'True': more text lines on the input list(s)
              LevelReader.CurrentTextLine:=LevelReader.InputLines.Text; // put the next text line from the list into the level reader buffer
              LevelReader.InputLines:=LevelReader.InputLines.Next; // advance to the next text line on the input list
              end;
           end;

        s:=LevelReader.CurrentTextLine; LevelReader.CurrentTextLine:=''; // get the next text line from the level reader buffer, if any
        Result:=ParseTextLine(s);
        end;

      if Generator.FixedTemplate then with Generator do
         // initialize the board generator before the candidates are loaded so the candidates can inherit fixed squares from the template board when they are loaded by 'GAAppendFromGame'
         InitializeBoardGenerator(Method,not Control.ForwardSearch,WallCount,BoxCount,RandomSeed,MinPushCount,MaxBoxCount,MaxOpenPositions,TemplatePlayerPos,StartNo,TailPushesThreshold,
                                  PrettyPrinting,MaxBoxCountEnabled,MaxOpenPositionsEnabled,TrivialOpeningPushesIgnored,TailPushesThresholdEnabled,RandomSeedEnabled,FixedTemplate,TemplateBoard);

      // load candidates
      try
        Level:=TLevel(SokoFile__.Levels.First);
        OpenForm.Game.Verbose:=False; Game.ReverseMode:=False; // 'ReverseMode=False': avoid that 'InitializeGame' drops unconnected goals even if the level type is set to 'connected goals'
        while Result and Assigned(Level) and (IndividualCount<High(Individuals)) do begin
          OpenForm.Game.SokoFile:=nil; // ensure that 'OpenForm.Game' doesn't attempt to save an old game, if any
          if OpenForm.Game.LoadFromFile(Level.Name,SokoFile__,True,s) then begin
             GAAppendFromGame(OpenForm.Game,not IsAClipboardImport,IsANewCandidateSet and (not IsAClipboardImport));
             end;

          Level:=TLevel(Level.Next);
          end;

        MainForm.Generator.SortCandidates(Control.FitnessFunction);

        for IndividualNo:=1 to GA.IndividualCount do GAMakeAllelesFromLevel(IndividualNo); {make alleles for the loaded levels}

      finally with OpenForm.Game do begin
                SokoFile:=nil; Clear; SokoFile:=MainForm.SokoFile; // restore 'OpenForm.Game.SokoFile'
                end;

      //Result:=Result and (FileName<>'');
      end;
...
{$IFDEF YASC}
  ...
  function  GAAppendFromGame(Game__:TSokoGame; IsACandidateSetMember__,UseCandidateNumbers__:Boolean):Boolean;  {Specific to Sokoban YASC}
  var ARow:Integer;
      Solution:TSnapshot; Snapshot:TExtendedSnapshotAsText;
  begin // precondition: the generator is not currently running in another thread
    Result:=Assigned(MainForm.Generator.PluginThread);
    if Result then begin
       MainForm.Generator.Enter;
       try
         with GA do begin
           Result:=IndividualCount<High(Individuals);
           if Result then
              try
                Inc(IndividualCount);
                Individuals[IndividualCount].CandidateNo:=0;
                Individuals[IndividualCount].Level:=nil;
                Individuals[IndividualCount].PushCount:=0;
                Individuals[IndividualCount].Score:=0;
                Individuals[IndividualCount].Fitness:=-1;

                // load statistics from level notes, if any
                LevelReader.InputLines:=Game__.Notes.Lines.First;
                while Assigned(LevelReader.InputLines) do begin
                  GAParseCandidateNotesTextLine(LevelReader.InputLines.Text,IsACandidateSetMember__,UseCandidateNumbers__,IndividualCount);
                  LevelReader.InputLines:=LevelReader.InputLines.Next; // advance to the next text line in the candidate notes
                  end;

                if Individuals[IndividualCount].CandidateNo=0 then begin
                   Inc(Statistics.CandidatesCount);
                   Individuals[IndividualCount].CandidateNo:=Statistics.CandidatesCount;
                   end;
                if (not IsACandidateSetMember__) and (Individuals[IndividualCount].TimeOfBirthMS=0) then
                   Individuals[IndividualCount].TimeOfBirthMS:=GACalculateElapsedTimeMS;

                // load best solution, if any
                if            IsACandidateSetMember__ and // only load solutions for candidate set members; other candidates added to the set must have their fitness calculated just like auto-generated candidates
                              (Game__.BoxCount<=MAX_BOX_COUNT) then begin // only load the solution if the level hasn't been truncated (the level may exceed the maximum number of boxes/goals)
                              if   Assigned (Game__.BestSolutionPushes) then
                                   Solution:=Game__.BestSolutionPushes
                              else Solution:=Game__.BestSolutionMoves;
                              if Assigned(Solution) and (Solution.PushCount>MAX_HISTORY_MOVES) then
                                 Solution:=nil;
                              end
                else          Solution:=nil;

                if            Individuals[IndividualCount].Fitness<0 then {no fitness specified; calculate one based on the number of pushes and score (box-lines and box-changes)}
                   if         Assigned(Solution) then
                              Individuals[IndividualCount].Fitness:=GAFitnessValue(GA.Control.FitnessFunction,Solution.PushCount,Solution.SecondaryScoreMetrics.BoxChanges+Solution.SecondaryScoreMetrics.BoxLines)
                   else  if   (Individuals[IndividualCount].PushCount<>0) and (Individuals[IndividualCount].Score<>0) then
                              Individuals[IndividualCount].Fitness:=GAFitnessValue(GA.Control.FitnessFunction,Individuals[IndividualCount].PushCount,Individuals[IndividualCount].Score)
                         else Individuals[IndividualCount].Fitness:=-1;

                if True //MainForm.Generator.IsActive or (not MainForm.Generator.IsActive)
                   then with Individuals[IndividualCount] do begin // put the new candidate on the pending queue without changing the existing members of the population and the queue
                   Alleles:=''; // the alleles are first created when the candidate is processed
                   MainForm.Generator.LoadLevel(Game__.BoardWidth,Game__.BoardHeight,-1,0,GACandidateName(IndividualCount),
                                                SokFile_.BoardToText(Game__.BoardWidth,Game__.BoardHeight,Game__.Board,''),
                                                Solution,False,False,Level);

                   end
                else with Individuals[IndividualCount] do begin // the generator is inactive; update the population and the candidate queue directly
                   {
                   Generator.MaxBoxCount:=MAX_BOX_COUNT; // so existing boards aren't truncated more than necessary
                   BoardWidth:=Game.BoardWidth; BoardHeight:=Game.BoardHeight;
                   Game.ReverseMode:=False; // 'ReverseMode=False': avoid that 'InitializeGame' drops unconnected goals even if the level type is set to 'connected goals'
                   Result:=// load board, i.e., convert it from the 2-dimensional external format to the internal 1-dimensional vector format
                           MakeLevelFrom2DimensionalBoard(Game__.BoardWidth,Game__.BoardHeight,0,0,Game__.Board);
                   if Result then begin
                      GAAddBoardToCandidateSet(IndividualCount,BoardWidth,BoardHeight);
                      MainForm.Generator.LoadLevel(Game.BoardWidth,Game.BoardHeight,-1,0,GACandidateName(IndividualCount),Alleles,
                                                   Solution,False,False,Level);
                      end;
                   }
                   end;

                if Result then with Individuals[IndividualCount] do
                   if   Assigned(Level) then begin
                        if   not Assigned(Solution) then       // 'True': no solution available for the candidate
                             Level.SnapshotsAsText.Clear;      // recycled levels may keep an unused snapshot as placeholder on the list; destroy it now
                        if   Level.SnapshotsAsText.IsEmpty and // 'IsEmpty': no solution available for the candidate; create a solution placeholder
                             CreateObject(otExtendedSnapshotAsText,TNode(Snapshot)) then begin
                             Snapshot.IsASolution                        :=True;
                             Snapshot.Metrics.MoveCount                  :=0; // number of moves not available
                             Snapshot.Metrics.PushCount                  :=PushCount;
                             Snapshot.Metrics.SecondaryMetrics.BoxLines  :=Max(0,Score) div 2; // box lines and box changes are only estimates, hence, it's ok simply to split them in 2
                             Snapshot.Metrics.SecondaryMetrics.BoxChanges:=Max(0,Score-Snapshot.Metrics.SecondaryMetrics.BoxLines);
                             Snapshot.MovesAsTextLines.AddBlankLine;
                             Level.SnapshotsAsText.Add(Snapshot);
                             end;
                        Snapshot:=TExtendedSnapshotAsText(Level.SnapshotsAsText.First);
                        if   Assigned(Snapshot) then begin
                             Snapshot.Tag           :=ChildrenCount;
                             Snapshot.TimeMS        :=TimeMS;
                             Snapshot.TimeOfBirthMS :=TimeOfBirthMS; // the time of birth belongs to the level, but it is easier to add it to the already existing 'TextendedSnapshotAsText' than creating a new extended level class
                             Snapshot.Metrics.SecondaryMetrics.PlayerLines:=Fitness; // store the fitess value in the player lines metric; the helps streamlining the implementation of various features, such as displaying and sorting the candidates
                             if Snapshot.MovesAsTextLines.IsEmpty then Snapshot.MovesAsTextLines.AddBlankLine;
                             if Snapshot.MovesAsTextLines.IsEmpty then raise Exception.Create(TEXT_MEMORY_FULL);
                             end
                        else raise Exception.Create(TEXT_MEMORY_FULL);

                        with ToolsForm.GeneratorTaskQueue do
       ...
       finally MainForm.Generator.Leave;
       end;
       end;
  end;

function  GAMakeAllelesFromLevel(IndividualNo__:Integer):Boolean;
var BoardWidth,BoardHeight:Integer;
begin // creates alleles for the candidate if it currently hasn't any; before processing, a candidate may be represented by its level only
  with GA do with Individuals[IndividualNo__] do begin
    try   Result:=False;

          if (Alleles='') and Assigned(Level) then begin
             if (not Level.BoardAsTextLines.IsEmpty) and
                (Length(Level.BoardAsTextLines.First.Text)=Level.Tag.BoardWidth*Level.Tag.BoardHeight) then begin
                BoardWidth:=Game.BoardWidth; BoardHeight:=Game.BoardHeight;

                if (Level.Tag.BoardWidth<>BoardWidth) or (Level.Tag.BoardHeight<>BoardHeight) then begin
                   InitializeBoard(Level.Tag.BoardWidth,Level.Tag.BoardHeight,False);
                   GA.AllelesCount:=Game.BoardWidth*Game.BoardHeight;
                   end;
                GAAllelesToBoard(Level.BoardAsTextLines.First.Text,Game.Board);

                GAAddBoardToCandidateSet(IndividualNo__,BoardWidth,BoardHeight);
                end;
             end;

          if   Length(Alleles)=GA.AllelesCount then
               Result:=True
          else if   ToolsForm.GeneratorTaskQueue.Levels[Level.Flags]=Level then
                    ToolsForm.GeneratorTaskQueue.Plugin.DeleteLevel(Level) // delete the candidate if creating the alleles failed
...
begin {$I-}
  ...
  if Result then with GA do begin
     ...
     {$IFDEF YASC}
       for i:=1 to IndividualCount do GAMakeAllelesFromLevel(i); {some candidates may be represented by their levels only; make their alleles now before calling 'GAWriteIndividualToFile()'}
       if StrEqual(ExtractFileExt(FileName),WORK_FILE_NAME_EXT) then
          FileName:=ChangeFileExt(FileName,TEMP_FILE_NAME_EXT); {avoid any file name clashes between the filename and the new temporary filename}
       TempFileName:=MakeNewFileName(ChangeFileExt(FileName,''),WORK_FILE_NAME_EXT,True);
       BakFileName :=MakeNewFileName(ChangeFileExt(FileName,''),BACKUP_FILE_NAME_EXT,True);
       if (TempFileName='') or (BakFileName='') then Result:=Error(DiskFullText,'');
     {$ELSE}
     ...
     if Result then begin
        ...
        for i:=1 to GA.IndividualCount do
            if Result then Result:=GAWriteIndividualToFile(i,GA.Individuals[i],F,
                                   {$IFDEF YASC}
                                     True  {'1': pretty-print all candidates}
                                   {$ELSE}
...
var i,Col,Row,BoxOrGoalMask,BoardSquareValue:Integer;
begin
  Result:=0;
  ...
  for i:=1 to Game.BoardSize do begin
      ...
      if (BoardSquareValue and WALL         )<>0 then begin
         ...
         if (Col>1) and (Col<Game.BoardWidth) and
            (Row>1) and (Row<Game.BoardHeight) then {only interior walls are considered}
...
var Col,Row,Square,RowOffset,PlayerPos:Integer;
begin
  with GA do with Individuals[0] do  begin {use item[0] as temporary item}
    ...
    AllelesCount:=Game.BoardWidth*Game.BoardHeight;
    ...
    for Row:=1 to Game.BoardHeight do begin {fill squares outside the exterior wall}
        ...
        while (Col<=Game.BoardWidth) and ((Game.Board[RowOffset+Col] and WALL)=0) do begin
          Alleles[Pred(Row)*Game.BoardWidth+Col]:=CH_WALL; Inc(Col);
          end;
        Col:=Game.BoardWidth;
        while (Col>0) and ((Game.Board[RowOffset+Col] and WALL)=0) do begin
          Alleles[Pred(Row)*Game.BoardWidth+Col]:=CH_WALL; Dec(Col);
          end;
        end;
...
begin
  ...
  {$IFDEF YASC}
    New1__.Individual.Level       :=nil; New2__.Individual.Level         :=nil;
  {$ELSE}
...
var i,oBoxCount,oHistoryCount,oPlayerPos {$IFDEF YASC},Len {$ENDIF}:Integer;
    oBoard:TBoard; oBoxPos:TBoxPositions;
    {$IFDEF YASC}
      s,s1:String; Snapshot:TExtendedSnapshotAsText;
    {$ENDIF}
begin {$I-}
  with Individual__ do begin
    Writeln(F__);
    ...
    {$IFDEF YASC}
      if Assigned(Level) and (not Level.SnapshotsAsText.IsEmpty) and
         (not TSnapshotAsText(Level.SnapshotsAsText.First).MovesAsTextLines.IsEmpty) and
         (TSnapshotAsText(Level.SnapshotsAsText.First).MovesAsTextLines.First.Text<>'') then begin
         Writeln(F__);

         Snapshot:=TExtendedSnapshotAsText(Level.SnapshotsAsText.First);

         s:=Snapshot.FormattedName(MainForm.Game.SecondaryMetricsInTitles);
         Writeln(F__,s);

         s:=Snapshot.MovesAsTextLines.First.Text;
         i:=0; Len:=Length(s);
         while i<Len do begin
           s1:=Copy(s,Succ(i),SOLUTION_LINE_LENGTH);
           Writeln(F__,s1);
           Inc(i,Min(SOLUTION_LINE_LENGTH,Len-i));
           end;
         end;
    {$ENDIF}
...
begin
  with Individual__ do with Individual do begin
    ...
    {$IFDEF YASC}
      if MainForm.Generator.ThreadState<>ptsProcessLevel then Fitness:=-1;
    {$ENDIF}
    Result   :=Fitness<>-1;
    end;
end;
...
begin
  ...
  {$IFDEF YASC}
    GA.FileName:='';
    Game.BoardWidth:=0; Game.BoardHeight:=0;
  {$ENDIF}
end;
...
{$IFDEF YASC}
  ...
  type TCandidateState=(csNewCandidate1,csTestedCandidate1,csNewCandidate2,csTestedCandidate2);
       TCandidateStateSet=set of TCandidateState;
  var  BoxPositionAddedToCandidateWithNoBoxes:Integer;
       CandidateHashKey:THashValue;
       CandidateState:TCandidateStateSet; New1,New2:TGAExtendedIndividual;

    procedure DeleteIndividual(IndividualNo__:Integer);
    var Level:TLevel;
    begin
      Level:=GA.Individuals[IndividualNo__].Level;
      if   Assigned(Level) then
           MainForm.Generator.DeleteLevel(Level)
      else GADeleteIndividual(IndividualNo__);
    end;

    function  AddIndividual:Boolean;
    var Snapshot:TExtendedSnapshotAsText;
    begin
      Result:=GA.IndividualCount<High(GA.Individuals);
      if Result then begin
         Inc(GA.IndividualCount);
         GA.Individuals[GA.IndividualCount].Alleles:='';
         FillChar(GA.Individuals[GA.IndividualCount],SizeOf(GA.Individuals[GA.IndividualCount]),0);
         GAAddBoardToCandidateSet(GA.IndividualCount,Game.BoardWidth,Game.BoardHeight);
         Inc(GA.Statistics.CandidatesCount);
         GA.Individuals[GA.IndividualCount].CandidateNo:=GA.Statistics.CandidatesCount;
         Result:=MainForm.Generator.LoadLevel(Game.BoardWidth,Game.BoardHeight,-1,0,GACandidateName(GA.IndividualCount),GA.Individuals[GA.IndividualCount].Alleles,
                                              nil,False,False,GA.Individuals[GA.IndividualCount].Level);
         if Result then with GA.Individuals[GA.IndividualCount].Level do begin
            if SnapshotsAsText.IsEmpty and
               CreateObject(otExtendedSnapshotAsText,TNode(Snapshot)) then
               SnapshotsAsText.Push(Snapshot);
            Result:=(SnapshotsAsText.Count=1) and
                    (TExtendedSnapshotAsText(SnapshotsAsText.First).MovesAsTextLines.SetSingleTextLine('')<>nil);
            end
         else begin
            Dec(GA.Statistics.CandidatesCount);
            end;
         if Result then with GA.Individuals[GA.IndividualCount] do begin
            Include(Level.Tag.Flags,ltfNew);
            Fitness:=-1; {'-1': not calculated yet}
            TimeOfBirthMS:=GACalculateElapsedTimeMS;
            TExtendedSnapshotAsText(Level.SnapshotsAsText.First).TimeOfBirthMS:=TimeOfBirthMS;
            MainForm.Generator.Level:=Level;
            end;
         end;
    end;

    function  FindUncalculatedCandidate(var BoxPositionAddedToCandidateWithNoBoxes__:Integer):Boolean;
    var IndividualNo,OriginalIndividualCount,Square:Integer;
    begin // finds the first uncalculated member of the population (with fitness = -1), if any; precondition: the task queue is blocked
      with GA do begin
        Individuals[0].Fitness:=0; // '0': vector element 0 is a sentinel for the backward search through the candidates
        repeat BoxPositionAddedToCandidateWithNoBoxes__:=-1;
               OriginalIndividualCount:=IndividualCount;
               IndividualNo:=Succ(IndividualCount);
               while Individuals[Pred(IndividualNo)].Fitness<0 do Dec(IndividualNo);
               Result:=IndividualNo<=IndividualCount;
               if Result then with Individuals[IndividualNo] do begin
                  Result:=Assigned(Level);
                  if   Result then begin
                       if (Alleles='') or (Length(Alleles)<>GA.AllelesCount) then
                          Result:=GAMakeAllelesFromLevel(IndividualNo); // if 'GAMakeAllelesFromLevel' fails it may delete the individual; that's the reason for checking 'OriginalIndividualCount' below in order not to delete it twice
                       if Result then begin {'True': the alleles are all right; now load the board and check whether the candidate is new or a duplicate}
                          New1.Individual:=Individuals[IndividualNo];
                          GAAllelesToBoard(New1.Individual.Alleles,New1.Board);
                          if Game.ReverseMode then begin
                             for Square:=0 to Game.BoardSize do New1.Board[Square]:=New1.Board[Square] and (not BOX);
                             for Square:=0 to Game.BoardSize do
                                 if New1.Board[Square] and GOAL<>0 then Inc(New1.Board[Square],BOX);
                             end
                          else begin
                             for Square:=0 to Game.BoardSize do New1.Board[Square]:=New1.Board[Square] and (not GOAL);
                             end;
                          LoadAndNormalizeBoard(New1.Board); {normalize the board before checking for duplicates}

                          if (Game.BoxCount=0) then begin {no boxes or goals on the board; try to put a box or a goal on the board}
                             GAMutation(New1.Board);
                             LoadAndNormalizeBoard(New1.Board);
                             if Game.BoxCount=1 then
                                BoxPositionAddedToCandidateWithNoBoxes__:=Game.BoxPos[1];
                             end;

                          Result:=((Game.BoxCount<>0)
                                   or
                                   Generator.FixedTemplate
                                  )
                                  and
                                  GAIsANewChromosome(New1.Board);
                          end;
                       if (not Result) and (IndividualCount=OriginalIndividualCount) then begin
                          DeleteIndividual(IndividualNo); // either it's a duplicate candidate, or creating the alleles for the level failed; delete the candidate
                          IndividualNo:=IndividualCount; // stay in the 'repeat' loop and search for another uncalculated level
                          end;
                       end
                  else begin
                    DeleteIndividual(IndividualNo); // malformed individual, i.e., it has no level; delete the individual
                    IndividualNo:=IndividualCount; // stay in the 'repeat' loop and search for another uncalculated level
                    end;
                  end;
        until  Result or (IndividualNo>IndividualCount);

        if Result then with MainForm.Generator do begin
           Level:=GA.Individuals[IndividualNo].Level;
           if   ltfSelected in Level.Tag.Flags then
                Level.Tag.Flags:=[ltfSelected] // keep the 'selected' flag; the synchronized 'TGeneratorThread.SynchronizeStartGeneratingLevel' resets it and updates the screen accordingly
           else Level.Tag.Flags:=[];
           Inc(GA.GenerationCount);
           end;
        end;
    end;

    function  MakeRandomLevel:Boolean;
    var CountDown:Integer;
    begin
      if (Game.BoardWidth<MIN_BOARD_WIDTH) or (Game.BoardHeight<MIN_BOARD_HEIGHT) then begin
         Game.BoardWidth :=GeneratorForm.SpinEditBoardWidth .Tag+2; // 'Tag' fields contain the user's confirmed values; fields like 'Value' and 'Checked' are current values when the user edits/views the settings
         Game.BoardHeight:=GeneratorForm.SpinEditBoardHeight.Tag+2;
         end;
      MakeBoard(Game.BoardWidth,Game.BoardHeight);
      with GeneratorForm do with Generator do begin
        WallCount:=(SpinEditInteriorWallsPct.Tag*Max(1,Game.BoardWidth-2)*Max(1,Game.BoardHeight-2)) div 100; // the spinedit contains the interior walls percentage; calculate the number of interior walls
        InitializeBoardGenerator(
          gmGA,
          not GA.Control.ForwardSearch,
          WallCount,
          Max(1,Max(BoxCount,MaxBoxCount)),
          Generator.RandomSeed,
          MinPushCount,
          SpinEditBoxesGoals.Tag,
          SpinEditOpenPositions.Tag*ONE_THOUSAND,
          TemplatePlayerPos,
          StartNo,
          SpinEditTailPushes.Tag,
          PrettyPrinting,
          CheckBoxBoxesGoalsLimitEnabled.Tag=Ord(True),
          CheckBoxOpenPositionsLimitEnabled.Tag=Ord(True),
          CheckBoxDiscardTrivialOpeningPushes.Tag=Ord(True),
          CheckBoxDiscountTailPushesEnabled.Tag=Ord(True),
          RadioButtonRandomSeed.Tag=Ord(True),
          FixedTemplate,
          Generator.Board);
          end;

      CountDown:=10000;
      repeat Dec(CountDown);
             Result:=GenerateNextBoard_Random;
             if Result then begin
                LoadAndNormalizeBoard(Game.Board); {normalize the board before checking for duplicates}
                Result:=GAIsANewChromosome(Game.Board);
                end;
      until  Result or
             (CountDown<=0) or
             (not Assigned(MainForm.Generator)) or
             (MainForm.Generator.ThreadState<>ptsProcessLevel);

      Result:=Result and AddIndividual;
      if Result then begin
         Inc(GA.GenerationCount);
         Generator.FixedTemplate:=False;    // reset the 'fixed template' flag when a random board is added to the candidate set; it doesn't make sense to treat it as a fixed template
         GA.Control.MutateWallsOnly:=False; // likewise, ensure that 'mutate walls only' is disabled
         end;
    end;

    function  MakeRandomLevels(Count__:Integer):Integer;
    var FirstLevel:TLevel;
    begin {precondition: the thread holds the generator lock}
      Result:=0;
      FirstLevel:=nil;
      while (Count__>0) and
            Assigned(MainForm.Generator) and
            (MainForm.Generator.ThreadState=ptsProcessLevel) and
            MakeRandomLevel do begin
            if not Assigned(FirstLevel) then
               FirstLevel:=MainForm.Generator.Level;
            Dec(Count__);
            Inc(Result);
            end;
      if    Assigned(FirstLevel) then
            MainForm.Generator.Level:=FirstLevel;
    end;

    function  CandidatePostprocessing(Level__:TLevel; BoxPositionAddedToCandidateWithNoBoxes__:Integer):Boolean; // precondition: 'Game.Board' and 'Game.History' contains the candidate starting position and the solution
    var IndividualNo,BoxNo:Integer; MoveAsText:String;

      function  CheckFixedTemplate:Boolean;
      var BoardSquareValue,Index,ItemCount:Integer;
      begin // check if the game board is a valid fixed template, if it's ok,
            // then save it and update the settings accordingly;
        ItemCount:=0;
        for Index:=0 to Game.BoardSize do
            if   Game.ReverseMode then
                 if (Game.Board[Index] and GOAL)<>0 then Inc(ItemCount)
                 else
            else if (Game.Board[Index] and BOX )<>0 then Inc(ItemCount);

        Result:=ItemCount>=0; //Result:=ItemCount<>0;

        if Result then begin
           for Index:=0 to Game.BoardSize do
               if Index<>BoxPositionAddedToCandidateWithNoBoxes__ then begin
                  BoardSquareValue :=Game.Board[Index] and (not FLAG_FIXED_SQUARE);
                  if   Game.ReverseMode then
                       if (BoardSquareValue and (WALL+GOAL      ))<>0 then Inc(BoardSquareValue,FLAG_FIXED_SQUARE)
                       else
                  else if (BoardSquareValue and (WALL+BOX+PLAYER))<>0 then Inc(BoardSquareValue,FLAG_FIXED_SQUARE);
                  Game.Board[Index]:=BoardSquareValue;
                  end;
           Generator.Board           :=Game.Board;
           Generator.FixedTemplate   :=True;
           Ga.Control.MutateWallsOnly:=(ItemCount>0) and (BoxPositionAddedToCandidateWithNoBoxes__<0);
           end
        else begin
           Generator.FixedTemplate   :=False;
    ...
    begin // 'CandidatePostprocessing'; returns 'True' if the new candidate should be added to the population; an actual replacement is performed later by a synchonized procedure
      try
        MainForm.Generator.Enter;
        try
                if GALookupLevel(GA.ParentLevel1,IndividualNo) then begin {update parent 1 if it still is a member of the population}
                   Inc(GA.Individuals[IndividualNo].ChildrenCount);
                   with TExtendedSnapshotAsText(GA.ParentLevel1.SnapshotsAsText.First) do Tag:=GA.Individuals[IndividualNo].ChildrenCount;
                   end;
                if GALookupLevel(GA.ParentLevel2,IndividualNo) then begin {update parent 2, if any, and if it still is a member of the population}
                   Inc(GA.Individuals[IndividualNo].ChildrenCount);
                   with TExtendedSnapshotAsText(GA.ParentLevel2.SnapshotsAsText.First) do Tag:=GA.Individuals[IndividualNo].ChildrenCount;
                   end;

                Result:=GALookupLevel(Level__,IndividualNo)
                        and
                        ((GA.Individuals[IndividualNo].Fitness>0) // '0': drop candidates with zero pushes
                         or
                         ((GA.Statistics.CandidatesCount=1) and // a fixed template may be a walls-only template with no boxes or goals
                          (GA.IndividualCount=1) and
                          Generator.FixedTemplate
                         )
                        )
                        and
                        ((IndividualNo<=GA.Control.PopulationSize) // 'True': the population isn't full
                         or
                         (GA.Individuals[IndividualNo].Fitness>GA.Individuals[GA.WorstIndividualNo].Fitness) // 'True': the new candidate is better than the worst member of the population
                         or
                         ((GA.IndividualCount>1) and
                          GA.Control.InactivityThresholdEnabled and
                          (GA.Statistics.LastNewCandidateNo+GA.Control.InactivityThreshold<GA.Statistics.CandidatesCount) // 'True': the population hasn't changed for some time
                         )
                        );
                if   Result then with GA.Individuals[IndividualNo] do begin // 'True': the new candidate should be added to the population
                     // update the level with the board and the solution; text versions of the new board and the solution are first created here where it's known that the candidate is qualified to be a member of the population
                     if (GA.Statistics.CandidatesCount=1) and (GA.IndividualCount=1) and Generator.FixedTemplate then
                        CheckFixedTemplate;                      // update information about fixed squares
                     GABoardToAlleles(Game.Board,Alleles);       // create text version of the new board
                     Level.BoardAsTextLines.First.Text:=Alleles; // update the level board
                     Level.Tag.BoardWidth:=Game.BoardWidth;      // update the level board dimensions
                     Level.Tag.BoardHeight:=Game.BoardHeight;    // update the level board dimensions

                     with TExtendedSnapshotAsText(Level.SnapshotsAsText.First) do with Metrics do begin // create a text version of the solution
                       if FirstMoveToText(PushCount,MoveAsText) then // make a text version of the solution, including intermediate non-pushing player-moves
                          repeat MovesAsTextLines.First.Text:=MovesAsTextLines.First.Text+MoveAsText;
                          until  not NextMoveToText(PushCount,MoveAsText);
                       MoveCount                             :=Length(MovesAsTextLines.First.Text);
                       PushCount                             :=Game.History.Count;
                       SecondaryMetrics                      :=Game.History.SecondaryMetrics;
                       SecondaryMetrics.PlayerLines          :=Game.History.Fitness; // the fitness is stored in 'PlayerLines'
                       GA.Individuals[IndividualNo].PushCount:=Game.History.Count;
                       GA.Individuals[IndividualNo].Score    :=Game.History.Score;
                       GA.Individuals[IndividualNo].Fitness  :=Game.History.Fitness;
                       IsASolution                           :=True;
                       for BoxNo:=1 to Game.BoxCount do
                           if (Game.Board[Game.BoxPos[BoxNo]] and GOAL)=0 then IsASolution:=False; // '0': the box isn't on a goal square, hence, something went wrong and the 'solution' isn't really a solution
                       GA.Individuals[IndividualNo].TimeMS   :=CalculateElapsedTimeMS(MainForm.Generator.StartTimeMS,GetTimeMS)-MainForm.Generator.PauseTimeMS; // update calculation time
                       TimeMS                                :=GA.Individuals[IndividualNo].TimeMS;
                       end;
                     end;
        finally MainForm.Generator.Leave;
        end;
      except on E:Exception do begin
                MainForm.Generator.PluginThread.SynchronizePluginError(E.Message);
                Result:=False;
                end;
      end;
    end;

    function  PrepareOffspring(ParentLevel1__,ParentLevel2__:TLevel; var New__:TGAExtendedIndividual):Boolean;
    begin // the offspring 'New__' has been created by 'GACrossOver' and is ready for mutations by 'GAMutation'; if the candidate isn't a duplicate then this function loads the candidate and returns 'True'
      GAMutation(New__.Board);
      LoadAndNormalizeBoard(New__.Board); {normalize the board before checking for duplicates}
      Result:=(Game.BoxCount<>0) and GAIsANewChromosome(New__.Board); {'True': the offspring is a new candidate; make it ready for processing}
      Result:=Result and AddIndividual;
    end;

    function  MakeOffspring(var CandidateState__:TCandidateStateSet; var New1__,New2__:TGAExtendedIndividual; var ParentLevel1__,ParentLevel2__:TLevel):Boolean;
    var No1,No2:Integer;
    begin
      Result:=False; CandidateState__:=[]; ParentLevel1__:=nil; ParentLevel2__:=nil;
      if (GA.IndividualCount>0) and (GA.IndividualCount<High(GA.Individuals)) then begin {otherwise, there is 1) no parents to create offspring from, or 2) not room for a new candidate}
         ...
         repeat
           ...
           if GACrossOver(No1,No2,New1__,New2__) then begin {'True': the cross-over produced 2 candidates}
              ParentLevel1__  :=GA.Individuals[No1].Level;
              if   (No1<>No2) and (No2<>0) then
                   ParentLevel2__  :=GA.Individuals[No2].Level
              else ParentLevel2__  :=nil;
              CandidateState__     :=[csNewCandidate1,csNewCandidate2]; {two candidates pending; they are not fully generated yet though; they still lack mutatations which is performed by 'PrepareOffspring'}
              end
           else begin {cross-over did not really take place; it merely selected a candidate for mutation}
              ParentLevel1__       :=GA.Individuals[No1].Level;
              ParentLevel2__       :=nil;
              CandidateState__     :=[csNewCandidate1]; {one candidate pending; it's not fully generated yet though; it still lacks mutatations which is performed by 'PrepareOffspring'}
              end;

           Exclude(CandidateState__,csNewCandidate1);    {'Exclude': the first candidate  has been selected for processing, it's not a pending candidate anymore}
           Result:=PrepareOffspring(ParentLevel1__,ParentLevel2__,New1__);
           if (not Result) and (csNewCandidate2 in CandidateState__) then begin
              Exclude(CandidateState__,csNewCandidate2); {'Exclude': the second candidate has been selected for processing, it's not a pending candidate anymore}
              Result:=PrepareOffspring(ParentLevel1__,ParentLevel2__,New2__);
              end;

           if (not Result) then begin
              MainForm.Generator.Leave;         // release the lock; this function has been called inside an 'Enter ... Leave' block
              try     SleepEx(0,False);         // ensure that the user gets a chance to terminate the job from the main thread
              finally MainForm.Generator.Enter; // acquire the lock again; this function has been called inside an 'Enter ... Leave' block, hence, upon return, the function must have acquired the lock
              end;
              end;
      ...
      if Result then Inc(GA.GenerationCount);
    end;

    function  CalculateFitness:Integer;
    var IndividualNo:Integer;
    begin
      Result:=GACalculateFitnessForChromosome(Game.Board);

      if Assigned(MainForm.Generator) then with MainForm.Generator do begin
         Enter;
         try     if ThreadState<>ptsProcessLevel then begin
                    Result:=-1;
                    if Positions.TotalDroppedCount<>0 then // 'True': the candidate has contributed to the number of 'memory full candidates'; remove the contribution
                       Dec(GA.Statistics.MemoryFullCandidatesCount);
                    end;
                 if GALookupLevel(Level,IndividualNo) then with GA.Individuals[IndividualNo] do
                    Fitness:=Result;
         finally Leave;
         end;
         end;
  ...
  begin // GARun; this is the main procedure for the generator thread
    with MainForm.Generator do begin
         try
           CandidateState:=[]; CandidateHashKey:=0;
           GA.ParentLevel1:=nil; GA.ParentLevel2:=nil;
           BoxPositionAddedToCandidateWithNoBoxes:=-1;
           repeat
             Enter;
             try     Level:=nil; StartTimeMS:=0; PauseTimeMS:=0; // pauses (e.g., when the user replays a solution on the screen) are deducted from the candidate calculation time
                     Game.ReverseMode:=not GA.Control.ForwardSearch;
                     if      FindUncalculatedCandidate(BoxPositionAddedToCandidateWithNoBoxes) then
                             CandidateState:=[] {drop a pending second candidate from a prior crossover, if any}
                     else if YASGen_.GA.Statistics.CandidatesCount<GA_DEFAULT_MAX_CANDIDATE_COUNT then {'True': the maximum number of candidates hasn't been exceeded yet}
                             if      (csNewCandidate2 in CandidateState) and {'True': a candidate is pending from a prior crossover}
                                     PrepareOffspring(GA.ParentLevel1,GA.ParentLevel2,New2) then begin {'True': after adding mutations to the candidate it's not a duplicate and now it's ready for processing}
                                     CandidateState:=[]; New1:=New2;
                                     end
                             else if MakeOffspring(CandidateState,New1,New2,GA.ParentLevel1,GA.ParentLevel2) {'MakeOffspring' creates one or two children depending on whether crossover is performed, and whether any of them are duplicates}
                                     or
                                     MakeRandomLevel {if the population is empty then create a random level}
                                     //(MakeRandomLevels(Max(1,GA.Control.PopulationSize-GA.IndividualCount))>=1) {if the population is empty then create random levels}
                                     then begin
                                     end;
                     ...
                     if   Assigned(Level) then begin
                          ...
                          CandidateHashKey:=GACalculateHashKey(Game.Board);
                          repeat StartTimeMS:=GetTimeMS;
                          until  StartTimeMS<>0;
                          end
                     else begin Level:=nil;                                          {set currently processed level to 'none'}
                                PluginThread.State:=Ord(ptsIdle);                    {nothing to do right now}
                          end;
             ...
             if Assigned(Level) and (ltfLocked in Level.Tag.Flags) then begin
                TGeneratorThread(PluginThread).SynchronizeStartGeneratingLevel;
                if   CalculateFitness>=0 then begin
                     GAHistoryAdd(CandidateHashKey);
                     if Assigned(Level) then begin // otherwise, something went wrong
                        Include(Level.Tag.Flags,ltfProcessed);
                        if      CandidatePostprocessing(Level,BoxPositionAddedToCandidateWithNoBoxes) then
                                Include(Level.Tag.Flags,ltfNew) // 'ltfNew': add the candidate to the population
                        else if Assigned(Level) then
                                Exclude(Level.Tag.Flags,ltfNew); // delete the candidate
                        end;
                     end
                else // calculated fitness < 0: the user terminated the task manually, the candidate level has boxes but no box pushes/pulls, or something went wrong
                     if Assigned(Level) then begin
                        Exclude(Level.Tag.Flags,ltfNew); // delete the candidate, unless the user terminated the task manually before the the candidate fitness has been calculated
                        Enter;
                        try     if MainForm.Generator.ThreadState=ptsProcessLevel then {'True': the user hasn't terminated the tast manually}
                                   Include(Level.Tag.Flags,ltfProcessed); {delete the candidate}
                        finally Leave;
                        end;
                        end;
                TGeneratorThread(PluginThread).SynchronizeStopGeneratingLevel;
                end;

           until MainForm.Generator.ThreadState<>ptsProcessLevel;

         except on E:Exception do begin
                if Assigned(PluginThread) then with PluginThread do begin
...
function  ColRowToSquare(Col__,Row__,ColCount__,RowCount__:Integer):Integer;
begin
  Result:=(Row__ * (ColCount__+2)) + Col__;
end;
...
begin
  ...
  for Row:=1 to Game.BoardHeight do
      if (Game.Board[ColRowToSquare(Col__,Row,Game.BoardWidth,Game.BoardHeight)] and (WALL+PLAYER+BOX+GOAL))<>0 then begin
         Result:=False; break;
         end;
end;
...
begin
  ...
  for Col:=1 to Game.BoardWidth  do
      if (Game.Board[ColRowToSquare(Col,Row__,Game.BoardWidth,Game.BoardHeight)] and (WALL+PLAYER+BOX+GOAL))<>0 then begin
         Result:=False; break;
         end;
end;
...
begin
  {$IFDEF YASC}
    SokUtil_.Error(Text__,GA_TEXT_APPLICATION_TITLE_LONG);
  {$ELSE}
...
function  GetCommandLineParameters(
            ...
            var StartNo__,RandomSeed__,MemoryByteSize__,MinPushCount__,
                MaxBoxCount__,
                MaxOpenPositions__,
                BoardWidth__,BoardHeight__,WallCount__,BoxCount__,
...
begin {a simple and not fool-proof implementation}
     ...
     if Result then
        case UpCase(s[2]) of
          ...
          'H'       : if        (Length(s)>=4) and (UpCase(s[4])='I') then
                                Result:=GetParameter(BoardHeight__,MIN_BOARD_WIDTH_AND_HEIGHT-2,MAX_BOARD_HEIGHT-2,0,ItemIndex)
                      else      Result:=False;
          ...
          'W'       : if        (Length(s)>=3) and (UpCase(s[3])='I') then
                                Result:=GetParameter(BoardWidth__,MIN_BOARD_WIDTH_AND_HEIGHT-2,MAX_BOARD_WIDTH-2,0,ItemIndex)
  ...
  if Result then begin
     if   BoardWidth__*BoardHeight__<>0 then begin
          Inc(BoardWidth__,2); Inc(BoardHeight__,2); {add exterior wall}
          Result:=((WallCount__+BoxCount__+1)<=(BoardWidth__-2)*(BoardHeight__-2)); {'+1': player}
...
begin
  ...
  {$ENDIF}
  {$IFDEF YASC}
    SokUtil_.Msg(Text__,GA_TEXT_APPLICATION_TITLE_LONG,0);
  {$ENDIF}
end;
...
function  PrepareBoardForPrettyPrinting(GuardAgainstTrimming__:Boolean):Integer;
  ...
  var i,j,Col,Row,FirstRow,LastRow:Integer; Direction:TDirection; HasNeighbourFloor:TBoardBoolean;
  begin
    with Game do begin
      Result:=0;
      ...
      for i:=0 to BoardSize do
          if PlayersReachableSquares[i]=PlayersReachableSquaresTimeStamp then begin
             for Direction:=Low(Direction) to High(Direction) do begin
                 ...
                 if (Direction=dUp) or (Direction=dDown) then begin {caution: hardwired to 4 directions only}
      ...
      if GuardAgainstTrimming__ then begin
         {ensure that the board won't be misaligned with other candidates as}
         {the result of trimming when the board is loaded}

         FirstRow:=0; {find the first non-empty row}
         for Row:=1 to BoardHeight do
             if   FirstRow=0 then begin
                  if not IsEmptyRow(Row) then FirstRow:=Row;
                  end
             else break;
         ...
         for Row:=1 to Pred(FirstRow) do begin
             ...
             if (Board[ColRowToSquare(1,Row,Game.BoardWidth,Game.BoardHeight)] and FLOOR)<>0 then Dec(FloorCount);
             Board[ColRowToSquare(1,Row,Game.BoardWidth,Game.BoardHeight)]:=WALL; Inc(Result);
             end;

         LastRow:=0; {find the last non-empty row}
         for Row:=BoardHeight downto 1 do
             if   LastRow=0 then begin
                  if not IsEmptyRow(Row) then LastRow:=Row;
                  end
             else break;

         Col:=1;
         while (Col<=BoardWidth) and IsEmptyCol(Col) do begin
           {the column is empty; add a wall to the bottom row to ensure correct alignment when the board is loaded}
           if (Board[ColRowToSquare(Col,LastRow,Game.BoardWidth,Game.BoardHeight)] and FLOOR)<>0 then Dec(FloorCount);
           Board[ColRowToSquare(Col,LastRow,Game.BoardWidth,Game.BoardHeight)]:=WALL; Inc(Result);
           Inc(Col);
           end;
         end;
      end;
...
procedure BoardToTextLines(var BoardTextLines__:TBoardTextLines; var FirstNonEmptyRow__,LastNonEmptyRow__:Integer);
var  Col,Row:Integer; Ch:Char; IsEmptyRow:array[0..MAX_BOARD_HEIGHT] of Boolean;
begin
  with Game do begin
    for Row:=1 to BoardHeight do begin
        IsEmptyRow[Row]:=True;
        BoardTextLines__[Row]:='';
        for Col:=1 to BoardWidth do begin
            Ch:=SquareToChar(ColRowToSquare(Col,Row,Game.BoardWidth,Game.BoardHeight));
            {$WARNINGS OFF}
              BoardTextLines__[Row]:=BoardTextLines__[Row]+Ch; {implicit string cast from 'ShortString' to 'String'}
            {$WARNINGS ON}
    ...
    while (FirstNonEmptyRow__<=LastNonEmptyRow__) and IsEmptyRow[FirstNonEmptyRow__] do Inc(FirstNonEmptyRow__);
    while (FirstNonEmptyRow__<=LastNonEmptyRow__) and IsEmptyRow[LastNonEmptyRow__ ] do Dec(LastNonEmptyRow__ );
    end;
end;

function  BoardToText(BoardWidth__,BoardHeight__:Integer; const Board__:TBoard; const LineSeparator__:String):String;
var Col,Row,Index,Index2,RowOffset:Integer;
begin
  SetLength(Result,(BoardWidth__*BoardHeight__)+(Max(0,Pred(BoardHeight__))*Length(LineSeparator__)*SizeOf(Char))); Index:=0;
  for Row:=1 to BoardHeight__ do begin
      RowOffset:=ColRowToSquare(0,Row,BoardWidth__,BoardHeight__);
...
{$IFDEF CONSOLE_APPLICATION}
  ...
  begin
    with Game do begin
      for Row:=1 to BoardHeight do begin
          for Col:=1 to BoardWidth do Write(Board[ColRowToSquare(Col,Row,Game.BoardWidth,Game.BoardHeight)]:3);
          Writeln;
          end;
      end;
  end;
    ...
    repeat
      ...
      for i:=0 to BoardSize do
          if (Board[i] and (WALL+BOX+GOAL))=0 then begin {for each non-goal, non-box floor-square...}
             ...
             if NeighbourFloorCount<=1 then begin {the floor-square is surrounded by 3 or 4 walls}
...
procedure MakeBoard(BoardWidth__,BoardHeight__:Integer);
...
begin {makes an empty board with a wall border}
  with Game do begin
    ...
    PlayerPos:=0; BoxCount:=0; GoalCount:=0; FloorCount:=(BoardWidth-2)*(BoardHeight-2);
    PlayersReachableSquaresTimeStamp:=High(PlayersReachableSquaresTimeStamp);
    BoardHashValue:=CalculateBoardHashValue;
    end;
end;

function  MakeLevelFrom2DimensionalBoard(BoardWidth__,BoardHeight__,MinBoardWidth__,MinBoardHeight__:Integer; const Board2D__:TBoard2D):Boolean;
var Col,Row,RowOffset,Square:Integer;
begin
  Result:=True;
  ...
  InitializeBoard(Max(BoardWidth__,MinBoardWidth__),Max(BoardHeight__,MinBoardHeight__),False);
  for Row:=1 to BoardHeight__ do begin
      RowOffset:=ColRowToSquare(0,Row,Game.BoardWidth,Game.BoardHeight);
      for Col:=1 to BoardWidth__ do Game.Board[RowOffset+Col]:=Board2D__[Col,Row] and (WALL+BOX+GOAL+PLAYER+FLOOR+FLAG_FIXED_SQUARE);
      end;
...
procedure ShowBoard;
{$IFDEF CONSOLE_APPLICATION}
  var Row,FirstRow,LastRow:Integer; BoardTextLines:TBoardTextLines;
  begin
    BoardToTextLines(BoardTextLines,FirstRow,LastRow);
    for Row:=FirstRow to LastRow do
        Writeln(BoardTextLines[Row]);
  end;
{$ELSE}
  begin
  end;
...
var Col,Row,RowOffset1,RowOffset2,NewBoardWidth,NewBoardHeight:Integer;
begin {removes redundant columns and rows from the board; returns 'True' if the board contained redundant columns and/or rows}
  PrepareBoardForPrettyPrinting(False);
  NewBoardWidth:=Game.BoardWidth; NewBoardHeight:=Game.BoardHeight;

  while (NewBoardWidth<>0) and IsEmptyCol(1) do begin
    for Row:=1 to NewBoardHeight do begin
        RowOffset1:=ColRowToSquare(0,Row,Game.BoardWidth,Game.BoardHeight);
  ...
  while (NewBoardHeight<>0) and IsEmptyRow(1) do begin
    for Row:=1 to Pred(NewBoardHeight) do begin
        RowOffset1:=ColRowToSquare(0,     Row ,Game.BoardWidth,Game.BoardHeight); {row offset}
        RowOffset2:=ColRowToSquare(0,Succ(Row),Game.BoardWidth,Game.BoardHeight); {next row offset}
  ...
  Result:=(NewBoardWidth<>Game.BoardWidth) or (NewBoardHeight<>Game.BoardHeight);

  if Result then begin
     for Row:=1 to NewBoardHeight do begin
         RowOffset1:=ColRowToSquare(0,Row,  NewBoardWidth,  NewBoardHeight); {co-ordinates with new dimensions}
         RowOffset2:=ColRowToSquare(0,Row,Game.BoardWidth,Game.BoardHeight); {co-ordinates with old dimensions}
         for Col:=1 to NewBoardWidth do Game.Board[RowOffset1+Col]:=Game.Board[RowOffset2+Col];
         end;
     InitializeBoard(NewBoardWidth,NewBoardHeight,False);
...
begin {$I-}
  with LevelReader do begin
    ...
    if IOResult=0 then begin
       ...
       Result:=LoadNextLevelFromFile(0,0);
...
function LoadNextLevelFromFile(MinimumBoardWidth__,MinimumBoardHeight__:Integer):Boolean; {a simple version, parsing board and optional heading titles only}
var i,j,LeftCol,LineLength,BoardWidth,BoardHeight:Integer; ErrorText:String;
    Board:TBoard2D;
  ...
  var i,j:Integer;
    ...
    begin {precondition: 'TextLine__' has been validated}
      ...
      for Col:=LeftCol__ to Pred(LeftCol__)+Length__ do
          Board[Succ(Col-LeftCol),BoardHeight]:=LevelReader.CharToBoardSquareValue[TextLine__[Col]];
    end;
  ...
  begin {ProcessCurrentTextLine}
    with LevelReader do begin
                 ...
                 else if BoardHeight>=MIN_BOARD_WIDTH_AND_HEIGHT then {done}
                         Result:=MakeLevelFrom2DimensionalBoard(BoardWidth,BoardHeight,Max(BoardWidth,MinimumBoardWidth__),Max(BoardHeight,MinimumBoardHeight__),Board)
                      else begin
  ...
  with LevelReader do begin
    ...
    if Result then begin
       ...
       for i:=0 to MAX_BOARD_WIDTH+1 do
           for j:=0 to MAX_BOARD_HEIGHT+1 do Board[i,j]:=FLOOR;

       {$IFDEF YASC}

          while  (not Result) and                                               {not done}
                 (ErrorText='') and                                             {no errors}
                 ((CurrentTextLine<>'') or Assigned(InputLines)) do begin       {not end-of-data}
            if   CurrentTextLine='' then begin {a line may be left over from one call to the next}
                 CurrentTextLine:=InputLines.Text;                              {put the next text line from the list into the level reader buffer}
                 InputLines:=InputLines.Next;                                   {advance to the next text line on the list}
                 end;
...
begin
  {$IFDEF YASC}
    FillChar(Game.History.SecondaryMetrics,SizeOf(Game.History.SecondaryMetrics),0);
    Game.History.Moves[0].BoxNo:=0;
  {$ENDIF}
  MoveNo__:=0;
...
begin
  ...
  if Result then with Game.History.Moves[MoveNo__] do begin
     ...
     if (BoxNo<>0) and
        ...
        {$IFDEF YASC}
          with Game.History.SecondaryMetrics do begin
            if      (MoveCount<>0) or (MoveNo__=1) then Inc(PushingSessions);
            if      BoxNo<>Game.History.Moves[Pred(MoveNo__)].BoxNo then begin
                    Inc(BoxChanges); Inc(BoxLines);
                    end
            else if Direction<>Game.History.Moves[Pred(MoveNo__)].Direction then
                    Inc(BoxLines);
            end;
        {$ENDIF}
        end
...
var {$IFDEF VERBOSE_OUTPUT}
      i,j,
    {$ENDIF}
    MoveNo,Row,oFloorCount,FirstRow,LastRow:Integer;
...
begin
  with Positions.OpenPositions do begin
    ...
    Result:=(Position__<>nil) {$IFDEF YASC} and (MainForm.Generator.ThreadState=ptsProcessLevel) {$ENDIF};
    if Result then begin
       Head:=Head^.Next;
...
{$IFDEF CONSOLE_APPLICATION}
  var i,Col,Row,MinPlayerPos:Integer;
  begin
...
var i,j,k,Mask:Integer; {$IFDEF YASC} OldBoard:TBoard; {$ENDIF}
begin
//Result:=GenerateNextBoard_EnumeratingAll; exit;
  ...
  if Result then with Generator do begin
     ...
     for i:=1 to Pred(PlayerIndex) do begin {'Pred': leave it to 'RunGenerator' to fill in the player position}
         ...
         j:=ColRowToSquare(Succ(Random(Game.BoardWidth)),Succ(Random(Game.BoardHeight)),Game.BoardWidth,Game.BoardHeight);
         k:=j;
     ...
     {$IFDEF YASC}
       Game.Board:=Board; // transfer the created random board to 'Game.Board'
       Board:=OldBoard;   // restore 'Generator.Board' which contains a template, if any
     {$ELSE}
       Game.Title      :=IntToStr(Game.BoardHeight-2)+'x'+IntToStr(Game.BoardWidth-2)+
                         '-w'+IntToStr(Generator.WallCount)+'-b'+IntToStr(Generator.BoxCount)+
...
begin
  with Generator do begin
    ...
    MidPoint                    :=ColRowToSquare(Succ(Game.BoardWidth) div 2,Succ(Game.BoardHeight) div 2,Game.BoardWidth,Game.BoardHeight);
...
var MinPlayerPos,OriginalPlayerPos,OriginalTailPushesThreshold:Integer;
  ...
  {$IFDEF YASC}
    ...
    begin
      TimeMS:=GetTimeMS;
      if CalculateElapsedTimeMS(GA.Control.LastShowStatusTimeMS,TimeMS)>=GA_DEFAULT_SHOW_STATUS_INTERVAL_TIME_MS then begin
         GA.Control.LastShowStatusTimeMS:=TimeMS;
         with MainForm.Generator do
           if ThreadState=ptsProcessLevel then PluginThread.PluginCallBackFunction;
         end;
    end;
  {$ENDIF}
  ...
  begin
    if Game.ReverseMode then
       ...
       {each player's access area separately, but it became obsolete when the  }
       {backward generator switched to handling all access areas simultaneously}
       Result:=True
    else begin
  ...
  begin
    with Position__^ do begin
      ...
      {$IFDEF CONSOLE_APPLICATION}
        if (Positions.OpenPositions.Count mod (100*ONE_THOUSAND))=0 then with Positions do begin
      ...
      {$ENDIF}
      {$IFDEF YASC}
        if (Positions.OpenPositions.Count mod (100*ONE_THOUSAND))=0 then ShowStatus;
      {$ENDIF}
...
//                   Positions.BestPosition^.Next:=nil;     {best position may still be a member of the open queue}
      ...
      with Position__^ do begin
        ...
        for i:=1 to MoveCount do
            if Result then with Moves[i] do begin             {for each move...}
               ...
               if MakePosition(BoardHashValue,MinPlayerPos,SuccessorDepth,SuccessorScore,BoxNo,Direction,Position__,Successor) then begin
                  {$IFDEF CONSOLE_APPLICATION}
                    if (Positions.Count mod (100*ONE_THOUSAND))=0 then begin
                       Write(Generator.Count);
                       if Generator.PlayerAccessAreaNo<>0 then Write('.',Generator.PlayerAccessAreaNo);
                  ...
                  {$ENDIF}
                  {$IFDEF YASC}
                    if (Positions.Count mod (100*ONE_THOUSAND))=0 then ShowStatus;
                  {$ENDIF}
                  end
               else if Successor=nil then begin               {'nil': memory was filled}
                       Result:=False;                         {exhaustive search failed}
  ...
  begin {TailPushesSearch}
   {$IFDEF YASC}
     MainForm.Generator.PluginThread.StatusText:=GeneratorStatusTailPositionsSearchText;
   {$ENDIF}
    ...
    {$IFDEF YASC}
       Positions.OpenPositions.Count:=0; {in YASC, dropped positions indicate memory full during processing, hence, clear open positions here before calling 'ClearOpenPositions' which counts any remaining positions as dropped positions}
    {$ENDIF}
...
begin {Search}
  with Game do begin
    ...
    if Result then begin
       {$IFDEF YASC}
         if   Game.ReverseMode then
              MainForm.Generator.PluginThread.StatusText:=TEXT_BACKWARD_SEARCH
         else MainForm.Generator.PluginThread.StatusText:=TEXT_FORWARD_SEARCH;
         ShowStatus;
       {$ENDIF}
    ...
    if (Positions.BestPosition<>nil) {$IFDEF YASC} and (MainForm.Generator.ThreadState=ptsProcessLevel) {$ENDIF} then with Positions.BestPosition^ do begin
...
function SearchPlayerAccessAreas(const Board__:TBoard):Integer; {Returns a 'fitness value' to the GA generator}
...
begin {SearchPlayerAccessAreas}
  with Game do begin
    ...
    until Game.ReverseMode or                                        {'Search' handles all player access areas when the backward generator is used}
          ...
          {$IFDEF YASC}
            (MainForm.Generator.ThreadState<>ptsProcessLevel) or
          {$ENDIF}
          (not GetNextPlayerAccessArea);                             {find next player access area, if any}

    if (History.Count<>0) {$IFDEF YASC} and (MainForm.Generator.ThreadState=ptsProcessLevel) {$ENDIF} then begin {'True': found a variation}
       {$IFNDEF YASC}
    ...
    {$IFDEF YASC}
      if   MainForm.Generator.ThreadState=ptsProcessLevel then
...
{$IFDEF YASC}
  ...
  initialization
    ...
    FillChar(LevelReader,SizeOf(LevelReader),0);
    FillChar(LevelStatistics,SizeOf(LevelStatistics),0);
    FillChar(Positions,SizeOf(Positions),0);
    FillChar(RandomState,SizeOf(RandomState),0);
    FillChar(GA,SizeOf(GA),0);
    GA.Control.MutationTypeAccumulatedFrequencies:=GA_DEFAULT_MUTATION_TYPE_ACCUMULATED_FREQUENCIES;
    InitializeLevelReader(True);
  finalization
    FinalizeGenerator;
...
{$ELSE}
  ...
  var
    ...
    begin {loads the next level and returns it in 'Generator.Board'}
      ...
      else begin Result:=LoadNextLevelFromFile(0,0);
                 Generator.Board:=Game.Board;
           end;
    end;
    ...
    begin {loads the first level and returns it in 'Generator.Board'}
      if LevelReader.InputFileName<>'' then begin
         ...
         MakeBoard(Game.BoardWidth,Game.BoardHeight); {initializes the board in global variable 'Game'}
         if Method=gmGA then Game.ReverseMode:=not GA.Control.ForwardSearch;
         InitializeBoardGenerator(Method,Game.ReverseMode,WallCount,BoxCount,RandomSeed,MinPushCount,MaxBoxCount,MaxOpenPositions,TemplatePlayerPos,StartNo,TailPushesThreshold,
...
{Application Toplevel}
  ...
  var BoardWidth,BoardHeight:Integer; InputFileName:String;
  begin
    FillChar(Positions,SizeOf(Positions),0);
    FillChar(Generator,SizeOf(Generator),0);
    FillChar(GA,SizeOf(GA),0);
    ...
    if Result then begin
       ...
       InitializeBoard(BoardWidth,BoardHeight,True); {save board dimensions, if any}
